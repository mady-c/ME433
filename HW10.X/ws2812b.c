...
//    T2CONbits.TCS = 0;
    ...
    //initialize output pin (B6) as low
//    ANSELB = 0;
    TRISBbits.TRISB6 = 0; //sets as output
    LATBbits.LATB6 = 0; //off (low)
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        for (j = 7; j >= 0; j--) {  // do it again for green
            // if the bit is a 1
            if((c[i].g>> j)&1 == 1) { //(c[i].g<<j== 1) {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) { // do it again for blue
            // if the bit is a 1
            if ((c[i].b>> j)&1 == 1) {//(c[i].b<<j == 1) {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
    ...
    for (i = 1; i <numBits; i++) {//(i = 1; i < numBits; i++) {
        while (TMR2 < delay_times[i]) {
        }
        LATBINV = 0b1<<6; // invert B6
